...
ID_SGSPU_S_PP = ['class', 'table table-bordered table-condensed table-scroll-thead table-free-cel']


def parse_sgspuS(url, teg):
...
def plan_priema_sgspuS():
    newsList = parse_sgspuS(URL_SGSPU_S, ID_SGSPU_S_PP)
    del newsList[:4]
...
# форматирование списка образовальных программ
def arrayFormatting_sgspuS():
    list_super_new = []
    newsList = parse_sgspuS(URL_SGSPU_S, ID_SGSPU_S_VI)
    del newsList[:2]
...
def availableToMe_sgspuS(subject):
    array_first = arrayFormatting_sgspuS()
    planList = plan_priema_sgspuS()
    out_all = {}
    for i in range(len(array_first)):
        if len(array_first[i]) == 22 and (subjectInRow_sgspuS(subject, array_first[i]) is True):
        ...
        elif len(array_first[i]) == 28 and (subjectInRow_sgspuS(subject, array_first[i]) is True):
            array_second = viewSubjectAndBall_sgspuS(array_first[i])
            out_all[array_first[i][0] + '_' + str(random.randint(0, MAX_INTEGER))] = {
                'code': str(array_first[i][0]),
                'program': str(array_first[i][2]),
...
# все образовательные программы СГСПУ - специалитет
def availableToAll_sgspuS():
    array_first = arrayFormatting_sgspuS()
    planList = plan_priema_sgspuS()
    out_all = {}
    for i in range(len(array_first)):
...
with open('src/sgspu_spec.json', 'w', encoding="utf-8") as fp:
    json.dump(availableToAll_sgspuS(), fp, ensure_ascii=False)
